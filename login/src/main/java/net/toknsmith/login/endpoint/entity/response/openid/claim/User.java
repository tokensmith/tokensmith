package net.toknsmith.login.endpoint.entity.response.openid.claim;
...
/**
 * This represents the id_token.
 * http://openid.net/specs/openid-connect-core-1_0.html#StandardClaims
*/
public class User extends Claims {
    ...
    public String toString() {
        ...
        if (this.email.isPresent()) {
            maskedEmail = this.email.get().substring(0, 2) + "****" + this.email.get().substring(email.get().length()-2, email.get().length());
        }

        return new StringBuilder()
            .append("User { ")
            .append("email = ").append(maskedEmail)
            .append(" }")
            .toString();
    }
}
